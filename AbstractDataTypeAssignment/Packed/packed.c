// Packed Boolean Array implementation
...
{
   if (!x || !s) { return false; }
   unsigned long* hashes = _hashes(s, x->capacity * BITS);
   unsigned char mask, check;
   for (int i = 0; i < K; i++) {
      mask = (BIGBIT >> _bit(hashes[i]));
      check = x->arr[_cell(hashes[i])] & mask;
      if (mask != check) {
         free(hashes);
         return false;
      }
...
dict* dict_init(unsigned int maxwords)
{
   ...
   unsigned long cap = SCALE(maxwords);
   d->capacity = (cap / BITS) + (cap % BITS ? 1 : 0);
   d->arr = (unsigned char*) ncalloc(d->capacity, sizeof(unsigned char));
   return d;
}
